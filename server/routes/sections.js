...
router.get('/:hospitalId/departments/:departmentId/sections', async (req, res) => {
  try {
...
router.put('/:hospitalId/departments/:departmentId/sections/:sectionId', async (req, res) => {
  try {
    ...
    res.json({ message: 'Section updated successfully', updatedSection: sectionToUpdate });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
router.delete('/:hospitalId/departments/:departmentId/sections/:sectionId', async (req, res) => {
  try {
    const { hospitalId, departmentId, sectionId } = req.params;
    ...
    const sectionIndex = department.sections.findIndex((section) => section._id.toString() === sectionId);
    if (sectionIndex === -1) {
      return res.status(404).json({ message: 'Section not found' });
    }
